...
export function startMqtt(opts) {
    ...
    client.on("message", async (topic, payload) => {
        if (topic !== env.MQTT_SENSOR_TOPIC)
            return;
        let parsedJson;
        try {
        ...
        try {
            ...
            await conn.query(`INSERT INTO devices (device_id, device_name, last_seen, status, ip_address)
         VALUES ($1, $2, NOW(), 'online', $3::inet)
         ON CONFLICT (device_id) DO UPDATE SET
           device_name = EXCLUDED.device_name,
           last_seen = NOW(),
           status = 'online',
           ip_address = COALESCE(EXCLUDED.ip_address, devices.ip_address)`, [msg.device_id, deviceName, msg.ip_address ?? null]);
            await conn.query(`INSERT INTO sensor_data (device_id, temperature, humidity, light, timestamp)
         VALUES ($1, $2, $3, $4, NOW())`, [msg.device_id, msg.temperature ?? null, msg.humidity ?? null, msg.light ?? null]);
            const logRows = [];
            ...
            for (const [sensorName, sensorValue] of [
                ...
                if (typeof sensorValue !== "number")
                    continue;
                const result = await conn.query(`INSERT INTO logs (device_id, device_name, ip_address, sensor_name, sensor_value, timestamp)
           VALUES ($1, $2, $3::inet, $4, $5, NOW())
           RETURNING device_id, device_name, ip_address::text as ip_address, sensor_name as sensor_type, sensor_value, timestamp`, [msg.device_id, deviceName, ip, sensorName, sensorValue]);
